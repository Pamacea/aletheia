...
import { getCachedPhilosophers, getCachedPhilosopherStats, getCachedMovementsForFilter } from '@/lib/cache/queries';
import { LinkOrnate } from '@/ui/components/LinkOrnate';
import { PhilosophersIcon, QuoteIcon } from '@/ui/icons/NavigationIcons';
...
export default async function PhilosophesPage() {
  ...
  const [philosophers, stats, movements] = await Promise.all([
    getCachedPhilosophers(),
    getCachedPhilosopherStats(),
    getCachedMovementsForFilter(),
  ]);
      ...
      <main className="w-full px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          <div className="mb-4 sm:mb-6">
            <span className="text-4xl sm:text-5xl lg:text-6xl living-word">Π</span>
          </div>
          ...
          <div className="mt-6 flex justify-center gap-4 sm:gap-8 text-sm flex-wrap">
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-12 h-12 border-2 border-sepia-600 bg-paper-200 flex items-center justify-center shrink-0">
                      <UserIcon className="w-6 h-6 text-sepia-600" />
                    </div>
                    <div className="min-w-0">
                      <h3 className="font-serif text-xl font-semibold text-ink group-hover:text-sepia-600 transition-colors break-words [hyphens:auto]" lang="fr">
                ...
                {philosopher.categories && philosopher.categories.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-3">
                    {philosopher.categories.slice(0, 3).map((category: string) => (
                      <span key={category} className="px-2 py-1 text-xs border border-sepia-600 bg-paper-200 text-sepia-600">
                        {category}
                      </span>
                    ))}
                ...
                {philosopher.works && philosopher.works.length > 0 && (
                  <div className="space-y-1">
                    <p className="text-sm font-medium text-ink mb-1">Oeuvres majeures :</p>
                    <ul className="text-sm text-ink-light space-y-0.5">
                      ...
                      {philosopher.works.length > 4 && (
                        <li className="text-sepia-600 text-sm italic">
                          +{philosopher.works.length - 4} autres oeuvres
                        </li>
                      )}
                    </ul>
                  </div>

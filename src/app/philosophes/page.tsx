...
import { PhilosopherCardSkeleton } from '@/components/philosophers';
    ...
    <div className="min-h-screen bg-paper-50">
      ...
      <main className="w-full px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="content-2-3">
          ...
          {/* Philosophers List */}
          <Suspense fallback={<PhilosopherGridSkeleton />}>
            <PhilosophersGrid />
          </Suspense>
        </div>
      </main>
...
// Client component for Suspense boundary
async function PhilosophersGrid() {
  const philosophers = await getPhilosophers();
...
function PhilosopherGridSkeleton() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {Array.from({ length: 8 }).map((_, i) => (
        <PhilosopherCardSkeleton key={i} />
      ))}
    </div>
  );
}
